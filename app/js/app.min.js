...
/* harmony import */ var _libs_form_elements_dropdown_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(5);
...
document.addEventListener('DOMContentLoaded', function () {
  // dropdown init
  var dropdowns = Array.from(document.querySelectorAll('.input-wrapper_dropdown'));
  dropdowns = dropdowns.map(function (dropdown) {
    return new _libs_form_elements_dropdown_js__WEBPACK_IMPORTED_MODULE_3__["default"]({
      // elems
      elem: dropdown,
  ...
  var likeButtons = Array.from(document.querySelectorAll('.input-wrapper__like-button'));
  ...
  var rateButtons = Array.from(document.querySelectorAll('.input-wrapper__rate-button'));
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('.input-wrapper__range-slider').each(function () {
    var slider = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).find('input');
    var from = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).find('.input-wrapper__range-from');
    var to = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).find('.input-wrapper__range-to');
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('.input-wrapper__pagination').pagination({
...
var Dropdown = /*#__PURE__*/function () {
  function Dropdown(options) {
    ...
    this.elem = options.elem; // serch elems
    ...
    this.dropdownActiveClassField = options.field.substring(1) + '_dropdown-active'; // other settings 

    this.isShowTotal = options.isShowTotal;
    this.initialized = false; // states

    this.defaultState = defaultState(this.options);
    this.state = this.defaultState.map(function (a) {
  ...
  _createClass(Dropdown, [{
    ...
    value: function handlerEventListeners(elem) {
      var _this = this;

      // Выпадание и схлопывание
      this.dropdownField.addEventListener('click', function (event) {
        _this.toggleDropdown();
      });
      document.addEventListener('click', function (event) {
        var ddElem = event.target.closest('.input-wrapper_dropdown');

        if (!ddElem || ddElem !== _this.elem) {
          _this.toggleDropdown('close');
        }
      }); // Изменение кол-ва и его сохранение

      this.options.forEach(function (option, idx) {
        var counterElements = option.querySelector(_this.counter).children;

        _this.count(counterElements, idx);
      }); // Сброс состояния
    ...
    value: function showTotal() {
      ...
      if (isShow) {
        ...
        this.state.forEach(function (el) {
          text = "".concat(text).concat(!text ? '' : ',', " ").concat(el.total, " ").concat(el.title);
        });
    ...
    value: function toggleDropdown() {
      ...
      } else this.dropdownField.classList.remove(this.dropdownActiveClassField);
    } // Изменить draftState и визуальную часть dropdawn

  }, {
    key: "count",
    value: function count(elements, index) {
      var _this2 = this;

      var minus = elements[0];
      var total = elements[1];
      var plus = elements[2];
      minus.addEventListener('click', function (event) {
        if (total.innerText >= 1) {
          total.innerText--;
          _this2.draftState[index] = total.innerText;
        }
      });
    ...
    value: function _default() {
      ...
      this.options.forEach(function (option, idx) {
        var total = option.querySelector(_this3.counter).children[1];
        total.innerText = _this3.defaultState[idx]['total'];
      });
    ...
    value: function save() {
      ...
      this.draftState.forEach(function (el, idx) {
        if (el !== null) {
          _this4.state[idx]['total'] = el;
        }
      });
...
function defaultState(options) {
  ...
  options.forEach(function (option) {
    var title = option.querySelector('.dropdown-select__title').innerText;
    var total = option.querySelector('.dropdown-select__total').innerText;
    state.push({
      title: title,
      total: total
    });
